...
第一步，先在项目中安装 `@machinaai/preset-ui`

```sh
$ yarn add @machinaai/preset-ui -D
```
...
- UI Server（@machinaai/ui/src/UmiUI.ts，使用 Express）
- UI 主框架（@machinaai/ui/client，使用 umi 构建）
- UI 插件集（@umijs/plugin-ui）
    - Dashboard 面板（./plugins/dashboard）
    - 配置插件（./plugins/configuration）
    - 资产插件（@machinaai/plugin-ui-blocks）
    - 任务插件（@machinaai/plugin-ui-tasks）
