...
/**
 * Determine whether a path is empty
 * As long as there is a file, it is not empty
 * @param {*} path
 */
...
/**
 * Check for conflicts between two dependencies
 * @param {*} blockDeps
 * @param {*} projectDeps
 */
...
/**
 * Remove duplicate dependencies, dependencies in projectDeps are removed from blockDeps
 * @param {*} blockDeps
 * @param {*} projectDeps
 */
...
export const getBlockGenerator = (api: IApi) => {
  ...
  return class BlockGenerator extends Generator {
    public isTypeScript;
    // Block source directory path (/ tmp)
    public sourcePath;
    public dryRun;
    // Directory relative to pages
    public path;
    // Target path for block addition
    ...
    constructor({ args, name }) {
      ...
      // This parameter is the index of the block. Tsx | js
      this.entryPath = null;
      // This parameter is the directory of the current block
      this.blockFolderPath = join(paths.absPagesPath, this.path);
      this.routes = args.routes || [];
      this.blockType = args.blockType;
      // Asset type: git or files
    ...
    async gitBlockWriting() {
      ...
      // If the route is repeated, re-enter
