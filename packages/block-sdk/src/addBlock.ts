...
export async function addBlock(args: AddBlockOption = {}, opts: AddBlockOption = {}, api: IApi) {
  ...
  if (!ctx.isLocal && ctx.repoExists) {
    try {
      ...
      signale.info('An error occurred, please try `umi block clear`');
    }
  }
  ...
  if (generator.needCreateNewRoute && api.userConfig.routes && !skipModifyRoutes) {
    ...
    // Currently _modifyBlockNewRouteConfig only supports configuration routing
    // In the future, you can do automatic write comment configuration, support convention routing
