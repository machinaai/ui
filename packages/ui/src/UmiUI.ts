...
export default class UmiUI {
  ...
  async openProject(key: string, service?: any, opts?: any) {
    ...
    if (!existsSync(project.path)) {
      throw new ActiveProjectError({
        title: {
          'es-ES': `El proyecto ${project.path} no existe.`,
    ...
    if (!isUmiProject(project.path)) {
      throw new ActiveProjectError({
        title: {
          'es-ES': `El proyecto ${project.path} no es un proyecto es inválido`,
          'en-US': `Project ${project.path} is not a valid project.`,
        },
        lang,
        actions: [BackToHomeAction],
      });
    ...
    if (process.env.BIGFISH_COMPAT && isUsingUmi(project.path)) {
      throw new ActiveProjectError({
        title: {
          'es-ES': `El proyecto ${project.path} es Umi，No abrirlo con Bigfish`,
    ...
    if (!process.env.BIGFISH_COMPAT && isUsingBigfish(project.path)) {
      throw new ActiveProjectError({
        title: {
          'es-ES': `El proyecto ${project.path} es Bigfishm, no abrirlo con Umi`,
    ...
    if (!this.developMode && service) {
        ...
        if (!semver.gt(version, process.env.BIGFISH_COMPAT ? '3.0.0' : '3.0.0')) {
          throw new ActiveProjectError({
            title: process.env.BIGFISH_COMPAT
              ? `La versión de Bigfish ($ {version}) del proyecto local es demasiado baja, actualice a @ alipay / bigfish @ 3.0 o superior，<a target="_blank" href="https://yuque.antfin-inc.com/bigfish/doc/uzfwoc#ff1deb63">Ver detalles</a>。`
              : {
                  'es-ES': `La versión Umi del proyecto（${version}）es muy baja，actualice a umi@3.0 o superior，<a target="_blank" href="https://umijs.org/zh/guide/faq.html#umi-%E7%89%88%E6%9C%AC%E8%BF%87%E4%BD%8E%EF%BC%8C%E8%AF%B7%E5%8D%87%E7%BA%A7%E5%88%B0%E6%9C%80%E6%96%B0">ver detalle</a>。`,
                  'en-US': `Umi version (${version}) of the project is too low, please upgrade to umi@3.0 or above, <a target="_blank" href="https://umijs.org/guide/faq.html#umi-version-is-too-low-please-upgrade-to-umi-2-9-or-above">view details</a>.`,
                },
            lang,
      ...
      try {
        ...
        if (isDepLost(e) || isPluginLost(e)) {
          throw new ActiveProjectError({
            title: {
              'es-ES': `Archivo de dependencias no encontrado`,
  ...
  async openProjectInEditor(
    ...
    if (!existsSync(launchPath)) {
      if (callback.failure) {
        const msg = {
          'es-ES': `Error al abrir el editor ${launchPath}el proyecto no existe`,
  ...
  async openConfigFileInEditor(projectPath: string, { success, failure, lang }) {
    ...
    try {
      assert(
        configFile,
        lang === 'es-ES'
          ? 'Falló al abrir el editor, el archivo de configuración no existe'
  ...
  async createProject(opts = {}, { onSuccess, onFailure, onProgress, lang }) {
    ...
    let createOpts = opts;
    if (key) {
      assert('retryFrom' in opts, `key with retryFrom Debe proporcionarse al mismo tiempo.`);
      ...
      if (!retryFrom) {
        ...
        setProgress({
          ...
          steps: {
            'es-ES': [
              'Validar Parametros',
              'Instalar o Actualizar create-umi',
